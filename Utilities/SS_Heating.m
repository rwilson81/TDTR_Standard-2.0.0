...
if nargin==0 % only used if not called as an external function
    absorbance =0.15; %Aluminum
    %abslayer =40;
    lambda=[100 0.1 1.3 100]; %W/m-K
    C=[2.4 0.1 1.6 1.6]*1e6; %J/m^3-K
    h=[75 1 300 1e6]*1e-9; %m ("Al" thickness estimated at 84nm=81nm Al+3nm oxide, from picosecond acoustic)
    eta=ones(1,numel(lambda)); %isotropic layers, eta=kx/ky;
    
    r=7.5e-6;
    r_pump=r; %pump 1/e^2 radius, m
    r_probe=r; %probe 1/e^2 radius, m
    A_tot_powermeter = 50*1e-3; %laser power (Watts) . . . (assumes the chopper isn't on!)
    t_rep = 12.5e-9;
